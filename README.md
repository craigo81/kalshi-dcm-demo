...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                              CFTC-COMPLIANT DCM ARCHITECTURE                     │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│  ┌──────────────────┐        ┌──────────────────┐        ┌──────────────────┐   │
│  │   Trading UI     │        │  Surveillance    │        │   External       │   │
│  │   (React/TS)     │        │  Dashboard       │        │   DCM APIs       │   │
│  │   Port 3000      │        │  Port 3001       │        │                  │   │
│  └────────┬─────────┘        └────────┬─────────┘        └────────┬─────────┘   │
│           │                           │                           │              │
│           │ HTTP/WS                   │ HTTP/WS                   │ HTTPS        │
│           ▼                           ▼                           ▼              │
│  ┌────────────────────────────────────────────────────────────────────────────┐ │
│  │                         GO BACKEND (Port 8080)                              │ │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐│ │
│  │  │ API Layer   │  │ Auth/JWT    │  │ WebSocket   │  │ Kalshi Client       ││ │
│  │  │ (handlers)  │  │ (CP 17)     │  │ Hub (CP 9)  │  │ (market data)       ││ │
│  │  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘  └──────────┬──────────┘│ │
│  │         │                │                │                     │           │ │
│  │         ▼                ▼                ▼                     │           │ │
│  │  ┌─────────────────────────────────────────────────────────────┐│           │ │
│  │  │              COMPLIANCE ENGINE (CP 2, 4, 5, 11)             ││           │ │
│  │  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐          ││           │ │
│  │  │  │ Surveillance│  │ Position    │  │ Pre-Trade   │          ││           │ │
│  │  │  │ (wash/spoof)│  │ Limits      │  │ Margin Check│          ││           │ │
│  │  │  └─────────────┘  └─────────────┘  └─────────────┘          ││           │ │
│  │  └─────────────────────────────────────────────────────────────┘│           │ │
│  │         │                                                       │           │ │
│  │         ▼                                                       │           │ │
...
│  │  │              DATA LAYER (CP 13, 18)                         ││           │ │
│  │  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐          ││           │ │
│  │  │  │ Mock Store  │  │ Persistence │  │ Audit Log   │          ││           │ │
│  │  │  │ (users,     │  │ (JSON/5yr)  │  │ (monthly    │          ││           │ │
│  │  │  │  wallets)   │  │             │  │  archives)  │          ││           │ │
│  │  │  └─────────────┘  └─────────────┘  └─────────────┘          ││           │ │
│  │  └─────────────────────────────────────────────────────────────┘│           │ │
│  └────────────────────────────────────────────────────────────────┼───────────┘ │
...
Data Flow:
1. User → Trading UI → Backend API → Compliance Check → Order → DCM
2. DCM → Market Data → WebSocket Hub → Trading UI (real-time)
3. Operator → Surveillance Dashboard → Alert/Halt APIs → Backend
```

### Component Responsibilities

| Component | Core Principles | Responsibilities |
|-----------|-----------------|------------------|
| **Trading UI** | CP 9 | User interface for trading, positions, wallet |
| **Surveillance Dashboard** | CP 4, 5 | Operator monitoring, halt controls, alerts |
| **API Layer** | CP 2 | RESTful endpoints, request validation |
| **Auth/JWT** | CP 17 | User authentication, KYC verification |
| **Compliance Engine** | CP 4, 5, 11 | Pre-trade checks, surveillance, limits |
| **Kalshi Client** | CP 3, 9 | Market data fetching, order routing |
| **Data Layer** | CP 13, 18 | Segregated funds, 5-year recordkeeping |
...
│   ├── cmd/server/main.go          # Go backend API
...
│   │   └── package.json
│   ├── static/index.html           # Legacy fallback UI
